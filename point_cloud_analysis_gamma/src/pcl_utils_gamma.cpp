...
#include <point_cloud_analysis_gamma/pcl_utils_gamma.h>
#include <pcl-1.7/pcl/PCLHeader.h>
//uses initializer list for member vars

PclUtilsGamma::PclUtilsGamma(ros::NodeHandle* nodehandle) : nh_(*nodehandle), pclKinect_ptr_(new PointCloud<pcl::PointXYZ>),
...
int PclUtilsGamma::read_clr_pcd_file(string fname)
{
...
//non-color version
int PclUtilsGamma::read_pcd_file(string fname)
{
...
Eigen::Affine3f PclUtilsGamma::make_affine_from_plane_params(Eigen::Vector3f plane_normal, double plane_dist) {
...
Eigen::Affine3f PclUtilsGamma::make_affine_from_plane_params(Eigen::Vector4f plane_parameters) {
...
Eigen::Affine3f PclUtilsGamma::make_affine_from_plane_params(Eigen::Vector4f plane_parameters, Eigen::Vector3f centroid) {
    Eigen::Vector3f plane_normal;
    Eigen::Affine3f A_transform;
    double plane_dist;
    plane_normal(0) = plane_parameters(0);
...
void PclUtilsGamma::fit_points_to_plane(Eigen::MatrixXf points_mat, Eigen::Vector3f &plane_normal, double &plane_dist) {
...
Eigen::Vector3f  PclUtilsGamma::compute_centroid(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr) {
...
Eigen::Vector3f  PclUtilsGamma::compute_centroid(pcl::PointCloud<pcl::PointXYZ> &input_cloud) {
    Eigen::Vector3f centroid;
    Eigen::Vector3f cloud_pt;   
...
/*
void PclUtilsGamma::fit_xformed_selected_pts_to_plane(Eigen::Vector3f &plane_normal, double &plane_dist) {
...
Eigen::Affine3f PclUtilsGamma::transformTFToEigen(const tf::Transform &t) {
    Eigen::Affine3f e;
 ...
 */
void PclUtilsGamma::transform_kinect_cloud(Eigen::Affine3f A) {
...
void PclUtilsGamma::transform_selected_points_cloud(Eigen::Affine3f A) {
...
void PclUtilsGamma::get_transformed_selected_points(pcl::PointCloud<pcl::PointXYZ> & outputCloud ) {
...
void PclUtilsGamma::get_copy_selected_points(pcl::PointCloud<pcl::PointXYZ>::Ptr &outputCloud ) {
...
void PclUtilsGamma::get_kinect_points(pcl::PointCloud<pcl::PointXYZ> & outputCloud ) {
...
void PclUtilsGamma::get_kinect_points(pcl::PointCloud<pcl::PointXYZRGB> & outputCloud ) {
...
void PclUtilsGamma::get_kinect_points(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &outputCloudPtr ) {
    int npts = pclKinect_clr_ptr_->points.size(); //how many points to extract?
...
void PclUtilsGamma::get_kinect_points(pcl::PointCloud<pcl::PointXYZ>::Ptr &outputCloudPtr ) {
    int npts = pclKinect_ptr_->points.size(); //how many points to extract?
...
void PclUtilsGamma::get_gen_purpose_cloud(pcl::PointCloud<pcl::PointXYZ> & outputCloud ) {
...
void PclUtilsGamma::get_selected_points(pcl::PointCloud<pcl::PointXYZ>::Ptr &outputCloudPtr ) {
    int npts = pclSelectedPoints_ptr_->points.size(); //how many points to extract?
...
void PclUtilsGamma::get_selected_points(pcl::PointCloud<pcl::PointXYZ> &outputCloud) {
    int npts = pclSelectedPoints_ptr_->points.size(); //how many points to extract?
    outputCloud.header = pclSelectedPoints_ptr_->header;
    outputCloud.is_dense = pclSelectedPoints_ptr_->is_dense;
    outputCloud.width = npts;
...
//void PclUtilsGamma::get_indices(vector<int> &indices,) {
//    indices = indicies_;
//}
...
void PclUtilsGamma::example_pcl_operation() {
...
void PclUtilsGamma::copy_indexed_pts_to_output_cloud(vector<int> &indices,PointCloud<pcl::PointXYZRGB> &outputCloud) {
...
Eigen::Vector3d PclUtilsGamma::find_avg_color() {
...
Eigen::Vector3d PclUtilsGamma::find_avg_color_selected_pts(vector<int> &indices) {
    Eigen::Vector3d avg_color;
    Eigen::Vector3d pt_color;
    //Eigen::Vector3d ref_color;
...
void PclUtilsGamma::find_indices_color_match(vector<int> &input_indices,
...
void PclUtilsGamma::filter_cloud_z(double z_nom, double z_eps, 
...
void PclUtilsGamma::find_coplanar_pts_z_height(double plane_height,double z_eps,vector<int> &indices) {
    filter_cloud_z(pclTransformed_ptr_,plane_height,z_eps,indices);
}

void PclUtilsGamma::filter_cloud_z(PointCloud<pcl::PointXYZ>::Ptr inputCloud, double z_nom, double z_eps, vector<int> &indices) {
...
void PclUtilsGamma::filter_cloud_z(PointCloud<pcl::PointXYZRGB>::Ptr inputCloud, double z_nom, double z_eps, vector<int> &indices) {
    int npts = inputCloud->points.size();
    Eigen::Vector3f pt;
    indices.clear();
    double dz;
...
void PclUtilsGamma::filter_cloud_z(PointCloud<pcl::PointXYZ>::Ptr inputCloud, double z_nom, double z_eps, 
                double radius, Eigen::Vector3f centroid, vector<int> &indices)  {
    int npts = inputCloud->points.size();
    Eigen::Vector3f pt;
    indices.clear();
...
void PclUtilsGamma::analyze_selected_points_color() {
    int npts = pclTransformedSelectedPoints_ptr_->points.size(); //number of points
    //copy_cloud(pclTransformedSelectedPoints_ptr_,pclGenPurposeCloud_ptr_); //now have a copy of the selected points in gen-purpose object
    //Eigen::Vector3f offset;
    //offset<<0,0,0.05;
...
void PclUtilsGamma::copy_cloud(PointCloud<pcl::PointXYZ>::Ptr inputCloud, PointCloud<pcl::PointXYZ>::Ptr outputCloud) {
...
void PclUtilsGamma::copy_cloud_xyzrgb_indices(PointCloud<pcl::PointXYZRGB>::Ptr inputCloud, vector<int> &indices, PointCloud<pcl::PointXYZRGB>::Ptr outputCloud) {
    int npts = indices.size(); //how many points to extract?
    outputCloud->header = inputCloud->header;
    outputCloud->is_dense = inputCloud->is_dense;
    outputCloud->width = npts;
...
void PclUtilsGamma::transform_cloud(Eigen::Affine3f A, pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr,
...
void PclUtilsGamma::transform_cloud(Eigen::Affine3f A, pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud_ptr,
...
void PclUtilsGamma::initializeSubscribers() {
    ROS_INFO("Initializing Subscribers");

    pointcloud_subscriber_ = nh_.subscribe("/kinect/depth/points", 1, &PclUtilsGamma::kinectCB, this);
    real_kinect_subscriber_ = nh_.subscribe("/camera/depth_registered/points", 1, &PclUtilsGamma::kinectCB, this);
    // add more subscribers here, as needed

    // subscribe to "selected_points", which is published by Rviz tool
    selected_points_subscriber_ = nh_.subscribe<sensor_msgs::PointCloud2> ("/selected_points", 1, &PclUtilsGamma::selectCB, this);
}

//member helper function to set up publishers;

void PclUtilsGamma::initializePublishers() {
 ...
 */
void PclUtilsGamma::kinectCB(const sensor_msgs::PointCloud2ConstPtr& cloud) {
...
void PclUtilsGamma::selectCB(const sensor_msgs::PointCloud2ConstPtr& cloud) {
